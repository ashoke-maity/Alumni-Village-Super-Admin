...
function Header({ onUserClick, onPostClick, onViewChange, currentView, onLogoClick }) {
  ...
  return (
    <header className="sticky top-0 z-50 w-full bg-red-500 border-b border-red-600 shadow-sm">
      <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
        <Link
          ...
          onClick={(e) => {
            if (onLogoClick) {
              e.preventDefault();
              if (currentView !== "feed" && onViewChange) {
                onViewChange("feed");
        ...
        >
          <img
            className="w-50 object-fit rounded-md"
            alt="Logo"
            src="/images/sof_tig_tiu_white.png"
          />
        </Link>
        ...
        <div className="hidden md:flex flex-1 justify-center ml-8">
          <div className="relative w-full max-w-md">
            <input
              type="text"
              placeholder="Search"
              className="w-full pl-10 pr-4 py-2 rounded-md border border-transparent text-gray-900 placeholder-gray-500 bg-white focus:outline-none focus:ring-2 focus:ring-white/20 transition-all shadow-sm"
        ...
        <div className="hidden md:flex items-center gap-6 text-white text-sm font-medium">
          <div className="relative" ref={notificationRef}>
            <button
              className="relative flex items-center gap-1 hover:text-gray-200 transition-colors"
              title="Notifications"
          ...
          <button
            onClick={() => {
              if (onViewChange) {
                onViewChange("find-people");
                navigate("/home?view=find-people");
              } else {
          ...
          >
            <div className="flex cursor-pointer">
            <UserSearch size={20} />
            <span className="hidden lg:inline px-1">Find People</span>
            </div>
          </button>
          ...
          >
            <div className="flex cursor-pointer">
            <Users size={20} />
            <span className="hidden lg:inline px-1">My Network</span>
            </div>
          </button>
          ...
          <div className="relative" ref={dropdownRef}>
            <button
              ...
              className="flex items-center gap-2 text-white rounded-full border border-red-400 hover:border-white transition-colors px-2 py-1 text-sm cursor-pointer"
            >
              <div className="bg-white rounded-full w-8 h-8 flex items-center justify-center text-red-600 font-semibold uppercase overflow-hidden">
                {profileImage ? (
                  <img
                    src={profileImage}
                    alt="Profile"
            ...
            {dropdownOpen && (
              <div className="absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg border text-sm z-50 py-1">
                <button
                  onClick={() => {
                    setDropdownOpen(false);
                    navigate('/profile');
                  }}
                  className="w-full text-left px-4 py-2 hover:bg-gray-50 cursor-pointer flex items-center gap-2 text-gray-700"
                >
                  <Users size={16} />
                  View Profile
                </button>
                ...
                {/* Need Help Dropdown Section - nested or separate? User asked for "Another dropdown".
                    Let's put Need Help as a separate top level item instead of inside user menu, 
                    but wait, I can just add a "Need Help" section here if that's what "Another dropdown" meant? 
                    "make another dropdown named 'Need Help' there". 
                    "There" usually implies the Navbar. 
                    I'll implement "Need Help" as a separate top-level component in the navbar.
                 */}
                <button
                ...
                searchResults?.users?.length > 0 && (
                  <>
                    {searchResults.users.map((user) => (
                      <li
                        key={user._id}
                        className="px-3 py-2 text-sm hover:bg-red-50 transition rounded flex items-center gap-2 cursor-pointer"
                      >
                        <Link
                          to="#"
