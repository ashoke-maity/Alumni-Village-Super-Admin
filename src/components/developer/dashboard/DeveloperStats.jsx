    ...
    const fetchStats = async () => {
      try {
        ...
        const [userRes, jobRes, eventRes, storyRes] = await Promise.all([
          axios.get(
            `${import.meta.env.VITE_DEVELOPER_API_URL}/user-count`,
            { headers: { Authorization: `Bearer ${token}` } }
          ),
          axios.get(
            `${import.meta.env.VITE_DEVELOPER_API_URL}/job-count`,
            { headers: { Authorization: `Bearer ${token}` } }
          ),
          axios.get(
            `${import.meta.env.VITE_DEVELOPER_API_URL}/event-count`,
            { headers: { Authorization: `Bearer ${token}` } }
          ),
          axios.get(
            `${import.meta.env.VITE_DEVELOPER_API_URL}/story-count`,
            { headers: { Authorization: `Bearer ${token}` } }
          ),
        ]);
        setStats({
